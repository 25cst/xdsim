...
#[test]
pub fn tick_not_gate_with_output() {
    let (index, res) = PackageIndexBuilder::new()
        .add_roots(&[dirs::data_dir().unwrap().join("xdsim/packages/components/")])
        .build();
    ...
    world
        .register_new_gate_output(RegisterNewGateOutput {
            socket: GateOutputSocket::new(not_gate, 0),
        })
        .unwrap();
...
pub fn tick_not_gate_multiple() {
    ...
    world
        .register_existing_gate_input(RegisterExistingGateInput {
            socket: GateInputSocket::new(not_gate, 0),
            buffer,
        })
        .unwrap();
